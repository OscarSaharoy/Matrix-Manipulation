...
class IO_Panel(gooey.Tk):
    ...
    def help(self):
        ...
        text = gooey.Message(box,text=helpstring,font=self.verdana_sml,width=x_res/2,fg='grey40')
    ...
    def click(self):
        ...
        if pressed[2]: # delete a point if right clicked
            ...
            for x in range(self.tri.shape[1]):
                ...
                if ((mx-px)**2 + (my-py)**2)**0.5 <= SP/2: # test if dist from mouse to centre of point is less than 3*SP
